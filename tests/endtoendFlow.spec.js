...
test.describe("End-to-End Flow", () => {
    ...
    test.skip("Validate Hamburger Menu Items and Logout", async ({ page, context }) => {
        console.log("\n[TEST] Validate Hamburger Menu Items and Logout");
        // Step 1: Login to the application
        console.log(`[STEP 1] Logging in with user: ${Config.testData.users.standard.username}`);
        await page.fill(Locators.login.username, Config.testData.users.standard.username);
        await page.fill(Locators.login.password, Config.testData.users.standard.password);
        ...
        // Step 3: Validate All Items menu item is visible and clickable
        console.log("[STEP 3] Validating 'All Items' menu item");
        const allItemsMenuItem = page.locator(Locators.menu.menuItemAllItems);
        await expect(allItemsMenuItem).toBeVisible();
        await expect(allItemsMenuItem).toBeEnabled();
        await expect(allItemsMenuItem).toHaveText('All Items');
        console.log("[✓] 'All Items' menu item is visible and enabled");
        
        // Click All Items and verify we're still on inventory page
        await allItemsMenuItem.click();
        await expect(page).toHaveURL(Config.urls.inventoryUrl);
        console.log("[✓] Clicked 'All Items', verified navigation to inventory page");
        
        // Reopen menu for next validation
        await page.click(Locators.menu.hamburgerButton);
        await expect(page.locator(Locators.menu.menuContainer)).toBeVisible();

        // Step 4: Validate About menu item is visible and clickable
        console.log("[STEP 4] Validating 'About' menu item");
        const aboutMenuItem = page.locator(Locators.menu.menuItemAbout);
        await expect(aboutMenuItem).toBeVisible();
        await expect(aboutMenuItem).toBeEnabled();
        await expect(aboutMenuItem).toHaveText('About');
        console.log("[✓] 'About' menu item is visible and enabled");
        
        // Click About - it may open in new tab or navigate away
        // Wait for potential new page or navigation
        console.log("[STEP 5] Clicking 'About' menu item");
        const [newPage] = await Promise.all([
            context.waitForEvent('page', { timeout: 5000 }).catch(() => null),
            aboutMenuItem.click()
        ]);
        
        // If new page opened, close it; otherwise navigate back
        if (newPage) {
            console.log("  - About opened in new tab, closing it");
            await newPage.close();
        } else {
            // Navigate back to inventory page if About navigated away
            console.log("  - About navigated away, returning to inventory page");
            await page.goto(Config.urls.inventoryUrl);
        }
        ...
        // Step 5: Validate Reset App State menu item is visible and clickable
        console.log("[STEP 6] Validating 'Reset App State' menu item");
        const resetMenuItem = page.locator(Locators.menu.menuItemResetAppState);
        await expect(resetMenuItem).toBeVisible();
        await expect(resetMenuItem).toBeEnabled();
        await expect(resetMenuItem).toHaveText('Reset App State');
        console.log("[✓] 'Reset App State' menu item is visible and enabled");
        
        // Click Reset App State (this resets the app state but doesn't navigate)
        console.log("[STEP 7] Clicking 'Reset App State'");
        await resetMenuItem.click();
        console.log("[✓] App state reset");
        
        // Wait a moment for reset to complete, then reopen menu for logout validation
        await page.waitForTimeout(500);
        await page.click(Locators.menu.hamburgerButton);
        await expect(page.locator(Locators.menu.menuContainer)).toBeVisible();

        // Step 6: Validate Logout menu item is visible and clickable
        console.log("[STEP 8] Validating 'Logout' menu item");
        const logoutMenuItem = page.locator(Locators.menu.menuItemLogout);
        await expect(logoutMenuItem).toBeVisible();
        await expect(logoutMenuItem).toBeEnabled();
        await expect(logoutMenuItem).toHaveText('Logout');
        console.log("[✓] 'Logout' menu item is visible and enabled");

        // Step 7: Click Logout and validate successful logout
        console.log("[STEP 9] Clicking 'Logout'");
        await logoutMenuItem.click();
        
        // Step 8: Verify logout was successful - should redirect to login page
        console.log("[STEP 10] Verifying successful logout");
        await expect(page).toHaveURL(Config.urls.baseUrl);
        console.log(`[✓] Redirected to login page: ${Config.urls.baseUrl}`);
        
        // Verify login elements are visible (confirming we're on login page)
        ...
        // Step 9: Verify we cannot access inventory page without logging in again
        console.log("[STEP 11] Verifying access control - attempting to access inventory page");
        await page.goto(Config.urls.inventoryUrl);
        // Should redirect back to login page
        await expect(page).toHaveURL(Config.urls.baseUrl);
        console.log("[✓] Access control verified: Inventory page redirects to login");
        console.log("[✓] Test passed: All hamburger menu items validated and logout successful");
    });
});